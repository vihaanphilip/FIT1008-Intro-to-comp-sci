...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        elif current.left is None: #min
            temp = current
            current = current.right
            return temp
        else:
            return self.get_min_aux(current.left, current)

    def get_min_aux(self, current, parent) -> TreeNode:
        if current.left is None: #base case: at min
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        """
            ...
            It should be a node having the smallest key among all the
            larger keys.
        """
        if current is None:
            raise ValueError("Heap is empty")
